...
import { TrendingUp, ThumbsUp, ThumbsDown, Medal, ArrowUp, ArrowDown, Minus, Trophy, Sparkles } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      {reputation && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <OverviewCard label="Total Reputation" value={reputation.reputationPoints.toLocaleString()} icon={Medal} color="text-primary" bg="bg-primary/10" />
          <OverviewCard label="Positive Feedback" value={reputation.positiveFeedbackCount.toLocaleString()} icon={ThumbsUp} color="text-emerald-600 dark:text-emerald-400" bg="bg-emerald-500/10" />
          <OverviewCard label="Negative Feedback" value={reputation.negativeFeedbackCount.toLocaleString()} icon={ThumbsDown} color="text-red-600 dark:text-red-400" bg="bg-red-500/10" />
          <OverviewCard label="Helpful" value={reputation.helpfulCount.toLocaleString()} icon={TrendingUp} color="text-amber-600 dark:text-amber-400" bg="bg-amber-500/10" />
        </div>
      )}
            ...
            {history.map((tx: any) => {
              ...
              return (
                <div key={tx.id} className="flex items-center gap-4 px-5 py-3.5">
                  <div className={`flex h-9 w-9 items-center justify-center rounded-lg ${color.replace("text-", "bg-")}/10`}>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      ...
                      {tx.sourceUser?.username && (
                        <> &middot; by {tx.sourceUser.displayName ?? tx.sourceUser.username}</>
                      )}
                    </p>
                  </div>
                  <span
