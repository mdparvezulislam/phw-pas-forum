...
export function OrderCard({ order, role }: { order: OrderWithRelations; role: "buyer" | "seller" }) {
  const otherUser = role === "buyer" ? order.seller : order.buyer;

  return (
    <Link
      href={role === "buyer" ? `/orders/${order.id}` : `/seller/orders/${order.id}`}
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0 flex-1">
          ...
          <h3 className="truncate text-lg font-semibold">{order.listing?.title ?? "Listing"}</h3>
          <div className="mt-1 flex items-center gap-4 text-sm text-muted-foreground">
            <span>
              {role === "buyer" ? "Seller" : "Buyer"}: {otherUser?.displayName ?? otherUser?.username ?? "Unknown"}
            </span>
