...
export function MembershipCard({
  ...
  const [cycle, setCycle] = useState<"MONTHLY" | "YEARLY" | "LIFETIME">(
    "MONTHLY",
  );
  ...
  const isVipPlus =
    plan.slug.toUpperCase() === "VIP_PLUS" ||
    plan.slug.toUpperCase() === "VIP+";
  ...
  if (isVip) {
    bgGradient =
      "from-indigo-950/40 via-zinc-900/90 to-indigo-950/40 border-indigo-500/30";
    ...
    bgGradient =
      "from-violet-950/40 via-zinc-900/90 to-violet-950/40 border-violet-500/30";
    ...
    bgGradient =
      "from-amber-950/40 via-zinc-900/90 to-amber-950/40 border-amber-500/35";
    ...
    bgGradient =
      "from-emerald-950/40 via-zinc-900/90 to-emerald-950/40 border-emerald-500/35";
    ...
    >
      ...
      <div>
        <div className="flex items-center justify-between">
          <span
            className={cn(
              "text-xs font-semibold tracking-wider uppercase border px-2 py-0.5 rounded-full",
              badgeColor,
            )}
          >
            {plan.badgeName}
        ...
        <h3
          className={cn(
            "mt-4 text-3xl font-extrabold tracking-tight bg-gradient-to-r bg-clip-text text-transparent",
            titleGradient,
          )}
        >
          {plan.name}
        ...
        <p className="mt-2 text-sm text-zinc-400 min-h-[40px]">
          {plan.description ||
            "Unlock premium platform benefits, priority support, and special items."}
        </p>
